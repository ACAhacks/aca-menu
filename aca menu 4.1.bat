...
if %car%==1 goto 15
if %car%==portscaner goto 16
if %car%==wifibrute goto 17
...
echo -------------------------------------- > ports_scan_result.txt
echo Open Ports on !targetIP!: >> ports_scan_result.txt
...
for %%p in (!ports!) do (
    ...
    if not errorlevel 1 (
        echo Port %%p is OPEN >> ports_scan_result.txt
        echo Port %%p is OPEN
    ) else (
...
echo Scan complete! Results saved in ports_scan_result.txt
pause
goto 3

:17
echo opening brute forcer
pause
bruteforcer
pause